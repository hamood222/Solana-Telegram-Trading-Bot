...
import { TOKEN_PROGRAM_ID, getAssociatedTokenAddress, getMint, unpackMint } from '@solana/spl-token';
...
export async function getSellTx(solanaConnection: Connection, wallet: Keypair, baseMint: PublicKey, quoteMint: PublicKey, amount: number, targetPool: string) {
  ...
  try {
    ...
    const quoteToken = new Token(TOKEN_PROGRAM_ID, quoteMint, 9)
    const baseTokenAmount = new TokenAmount(baseToken, Math.round(amount * (10 ** tokenBal.value.decimals)))
    // const baseTokenAmount = new TokenAmount(baseToken, Math.round(amount))
    ...
    const { swapTransaction }: any = await (
      await fetch("https://quote-api.jup.ag/v6/swap", {
        ...
        body: JSON.stringify({
          ...
          prioritizationFeeLamports: 152000
        }),
      })
    ).json();
